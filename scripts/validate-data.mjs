...
function checkDuplicateLabels(entries, path) {
  ...
  entries.forEach((entry, index) => {
    ...
    failures.push(`${path}: libellé dupliqué "${nom}" aux entrées #${seen.get(nom) + 1} et #${index + 1}`);
  });
}
...
fullEntries.forEach((entry, index) => {
  ...
  } else if ((entry.recette_meur === null) !== (entry.recette === null || entry.recette === undefined)) {
    failures.push(
